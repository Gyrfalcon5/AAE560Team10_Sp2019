classdef Link < handle
    ...
    properties (Dependent)
        ...
        driving_weight % Like travel time but it also includes the nodes 
                       % for driving
        walking_weight % Like driving weight but for walking       
    ...
    methods
        ...
        function time = get.driving_weight(obj)
            time = obj.travel_time +...
                (obj.nodes(1).wait_time + obj.nodes(2).wait_time)/2;
        end
        
        function time = get.walking_weight(obj)
            time = obj.travel_time * 6 + ...
                (obj.nodes(1).wait_time + obj.nodes(2).wait_time)/2;
        end
